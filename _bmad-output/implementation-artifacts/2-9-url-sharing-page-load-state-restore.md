...
3. **Given** a URL with partial or missing params, **When** the page loads, **Then** missing params fall back to Miami defaults (existing `decode_state` fallback logic — no new code needed).
4. **Given** a URL with an invalid `yr` value (not in `[5, 10, 15, 20, 25, 30]`), **When** the page loads, **Then** the horizon slider defaults to `defaults.HORIZON_YEARS` (10) — prevents a select_slider crash.
...
- [x] **Task 1: Extend `url_state.py` — add 'dp' and 'yr' params (AC: 5)**
  ...
  - [x] Verify `defaults.DOWN_PCT` and `defaults.HORIZON_YEARS` exist (they do — added in Story 2.5)

- [x] **Task 2: Add 3 new tests for 'dp' and 'yr' (AC: 6)**
  - [x] `test_decode_horizon_years_as_int` — `decode_state({'yr': '15'})` → `HORIZON_YEARS == 15` and is `int`
  - [x] `test_decode_down_pct_as_float` — `decode_state({'dp': '10.0'})` → `DOWN_PCT == 10.0` and is `float`
  - [x] `test_roundtrip_dp_and_yr` — roundtrip preserves DOWN_PCT and HORIZON_YEARS values
  - [x] Run `python -m pytest tests/ -v` — confirm 102 tests pass

- [x] **Task 3: Wire URL state into `app.py` (AC: 1–4)**
  ...
  - [x] Add horizon guard: if `_initial['HORIZON_YEARS'] not in [5, 10, 15, 20, 25, 30]` → reset to `defaults.HORIZON_YEARS`
...
- [x] **Task 4: Regression and smoke check (AC: 6)**
  ...
  - [x] Manual smoke: `streamlit run app.py` — move any slider, observe URL updates in the browser address bar
  - [x] Copy the URL, open it in a new tab — all sliders restore to the shared values, outputs match
...
C:\Users\criss\Documents\Home Buying\
  url_state.py       ← MODIFY (add 'dp' and 'yr' to PARAM_MAP; 'yr' to INT_PARAMS)
  tests/test_url_state.py ← MODIFY (add 3 new tests)
  app.py             ← MODIFY (add import, decode on load, slider value=_initial, encode after sidebar)
  defaults.py        ← DO NOT TOUCH (DOWN_PCT and HORIZON_YEARS already present)
...
### Task 1 — Exact changes to `url_state.py`
...
`select_slider(value=...)` requires the value to be the same type as the options. Options are `[5, 10, 15, 20, 25, 30]` — all ints. If 'yr' were decoded as float (10.0), `value=10.0` with int options would crash. `INT_PARAMS` ensures `decode_state` returns `int('10')` = `10`, not `float('10')` = `10.0`.

**Why 'dp' stays as float (not in INT_PARAMS):**
The down payment slider uses `min_value=3.0`, `max_value=30.0`, `step=0.5` — all floats. `value=20.0` is the expected type.
...
```
After adding 'dp'/'yr', this automatically includes `DOWN_PCT=20.0` and `HORIZON_YEARS=10` — all 6 tests using `_miami_defaults()` now cover the new params without modification. The 99 existing tests all continue to pass.

**One edge case in `test_decode_all_non_int_params_are_floats`:**
This test passes `'1.0'` for every param key including 'yr'. For 'yr' in INT_PARAMS, `decode_state` tries `int('1.0')` → `ValueError` → falls back to `defaults.HORIZON_YEARS`. The test only checks `if short_key not in INT_PARAMS`, so HORIZON_YEARS is excluded from the float assertion. Test passes ✓.

### Task 2 — Exact new tests (add to end of `tests/test_url_state.py`)
...
### Task 3 — Exact changes to `app.py`

**Step A: Add import** — after `import pandas as pd`:
```python
import url_state
```

**Step B: Replace the 4-line comment block and add decode call** — the current code above `with st.sidebar:` reads:
```python
...
**Step C: Replace all `value=defaults.CONST_NAME` with `value=_initial['CONST_NAME']`** — full mapping:
...
**Step D: Add URL write after the sidebar block** — immediately after `with st.sidebar:` closes (before `# ── Rent vs Buy Two-Path Calculation`):
...
In Streamlit 1.30+, writing to `st.query_params` updates the browser URL silently — it does NOT trigger a page rerun. The URL reflects the current state; users can copy and share it. Opening that URL triggers a fresh load where `st.query_params.to_dict()` returns the saved params.
...
|---|---|
| 3.3 | Sidebar polish — may add/remove "Miami defaults loaded" caption based on URL params |
| 3.x | All polish stories benefit from shareable URLs — scenarios can be shared exactly |
...
- 2026-05-30: Story 2.9 created — URL sharing & page load state restore
