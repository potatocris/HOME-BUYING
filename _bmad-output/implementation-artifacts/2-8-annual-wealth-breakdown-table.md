...
4. **Given** the "Better" column, **Then** it shows plain text "Renting" or "Buying" â€” no color coding, no badges (outcome neutrality).
...
- [x] **Task 2: Build and render the table (AC: 1â€“5)**
...
- [x] **Task 3: Regression check (AC: 6)**
  - [x] Run `python -m pytest tests/ -v` â€” all 99 tests pass (0 regressions)
...
- [x] **Task 4: Syntax and smoke check**
  ...
  - [x] Manual smoke: `streamlit run app.py` â€” table renders below chart, correct columns, correct row count
  - [x] Verify row count matches horizon slider (10yr â†’ 10 rows, 30yr â†’ 30 rows)
...
C:\Users\criss\Documents\Home Buying\
  app.py          â† MODIFY (add import + _fmt_dollar helper + table block)
  defaults.py     â† DO NOT TOUCH
  calculations.py â† DO NOT TOUCH
  url_state.py    â† DO NOT TOUCH
  requirements.txt â† DO NOT TOUCH (pandas==3.0.3 already present)
  tests/          â† DO NOT TOUCH (99 tests must stay green)
...
### Current state of `app.py` (Story 2.7 output â€” what you are editing)
...
# â† ADD: import pandas as pd

def _headline_card(...) -> str:
    ...
# â† ADD: _fmt_dollar() helper here
...
# â”€â”€ Wealth over time chart â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# ... Plotly chart already rendered via st.plotly_chart(...) ...

# Story 2.8 adds the annual breakdown table here.    â† REPLACE THIS COMMENT
```

**Variables in scope when the table block runs:**
| Variable | Type | Value at defaults (20% down, 10yr) |
|---|---|---|
| `renter_annual` | list[float] | 10 values, last â‰ˆ $240,604 |
| `buyer_annual` | list[float] | 10 values, last â‰ˆ $199,712 |
...
`pandas==3.0.3` is already in `requirements.txt` â€” no installation needed.

### Task 1 â€” `_fmt_dollar()` helper
...
### Task 2 â€” Exact table code (replaces `# Story 2.8 adds the annual breakdown table here.`)

```python
# â”€â”€ Annual wealth breakdown table â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
table_rows = []
...
**Difference sign convention:** `diff = r - b` â€” positive means renting ahead, negative means buying ahead. This matches the HeadlineCard logic (`final_renter - final_buyer`) so the two displays are consistent.

**"Better" tie-breaking:** `r >= b` â†’ "Renting" (same as the headline card's `final_renter >= final_buyer` â†’ winner = "Renting"). Ties go to Renting in both places â€” consistent.
...
`st.table()` renders a fully static HTML table â€” no hover, no scroll for large horizons (30yr = 30 rows). `st.dataframe()` handles long tables with scrolling and has better visual consistency with the rest of the Streamlit UI. Both accept DataFrames; `st.dataframe` is the right choice here.

### No new tests required

`_fmt_dollar()` is pure string formatting â€” simple enough that unit tests aren't warranted. The underlying data (`renter_annual`, `buyer_annual`) is already covered by 99 existing tests. Validation gates:
...
|---|---|
| 2.9 | URL state; table updates automatically when sliders restore from URL â€” no table changes needed |
| 3.5 | Chart & Table polish â€” may add column config, conditional formatting, or styling on top of 2.8 base |

### Learning from Story 2.7 (chart text visibility)

Story 2.7 had an issue where Plotly chart text was dim against the white background. The fix was `template="simple_white"` and explicit `font=dict(color="#1A1D2E")` on every layout component. For `st.dataframe`, Streamlit renders natively and inherits the app theme â€” no custom color fixes needed.
...
- `app.py` (modified â€” added import, `_fmt_dollar()` helper, table block)

### Change Log

- 2026-05-30: Story 2.8 created â€” annual wealth breakdown table
- 2026-05-30: Story 2.8 implemented â€” `_fmt_dollar()` helper, `pd.DataFrame` table with Year/Rent+Invest/Buy+Invest/Difference/Better columns; 99 tests passing
