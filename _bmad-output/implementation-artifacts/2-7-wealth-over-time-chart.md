...
8. **Given** the timeline slider or any input changes, **When** the page reruns, **Then** the chart updates in real time (Streamlit re-runs automatically â€” no extra code needed).
...
- [x] **Task 2: Build the Plotly figure (AC: 1â€“7)**
...
- [x] **Task 4: Syntax and smoke check**
  ...
  - [x] Manual smoke: `streamlit run app.py` â€” chart renders below headline, two lines visible, hover shows dollar values
  - [x] Move slider from 10yr to 30yr â€” chart extends to 30 data points, x-axis rescales
...
C:\Users\criss\Documents\Home Buying\
  app.py          â† MODIFY (add import + chart block)
  requirements.txt â† MODIFY (add plotly==6.7.0)
...
### Current state of `app.py` (Story 2.6 output â€” what you are editing)

```python
import streamlit as st
import defaults
import calculations
# â† ADD: import plotly.graph_objects as go
...
upfront_cash  = calculations.calculate_upfront_cash(...)   # â† needed for Year 0 renter start
...
# Story 2.7 adds the Plotly chart here.    â† REPLACE THIS COMMENT
...
### Task 1 â€” requirements.txt change
...
### Task 2 â€” Exact chart code (replaces the `# Story 2.7 adds the Plotly chart here.` comment)
...
- Renter's Year 0 wealth = `upfront_cash` â€” they kept the capital they didn't spend on a down payment
- Buyer's Year 0 wealth = `0.0` â€” they spent all their capital on the purchase (down payment + closing costs + furniture); home equity = 0 at time of purchase (paid market price)
...
`upfront_cash` is already computed in the calculation block before the chart â€” no new calculation needed.
...
`fig.add_vline(x, line_dash, line_color, annotation_text, annotation_position)` is stable since Plotly 5.0 (installed: 6.7.0). The `annotation_position="top"` places the label at the top of the vertical line. No additional imports needed â€” `go.Figure()` provides this method.
...
`calculations.py` is untouched. The chart code uses only list construction and Plotly â€” no custom business logic. Validation gates:
...
|---|---|
| 2.8 | Adds annual table directly below chart; consumes same `renter_annual`, `buyer_annual`, `horizon_years` â€” all already in scope |
| 2.9 | URL state; chart updates automatically when sliders restore from URL â€” no chart changes needed |
| 3.5 | Chart & Table polish â€” styling refinements on top of 2.7 base (line thickness, hover templates, etc.) |
...
- `app.py` (modified â€” added import + chart block)
- `requirements.txt` (modified â€” added plotly==6.7.0)

### Change Log

- 2026-05-30: Story 2.7 created â€” Plotly wealth-over-time line chart
- 2026-05-30: Story 2.7 implemented â€” two-line chart with Year 0 anchor, break-even vline, outcome-neutral colors, 99 tests passing
